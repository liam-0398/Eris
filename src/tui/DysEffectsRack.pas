...
{ TDysWaveformContent/TDysBottomPane (the FV bottom-dock waveform display
  and its mouse-drag selection/Ctrl+C/V/D/Delete mechanism, plus
  DysCopyWaveformSelection/DysGetWaveClipboard/ActiveBottomPane) were
  removed - ported to DysMptiApp.pas's own independent DrawWaveformPane
  (keyboard cursor/grid/selection, not mouse-drag) and unreachable here
  regardless (dysnomia.lpr boots RunDysnomiaMpti, not TDysnomiaApp, the
  only caller either ever had). src/tui-freevision keeps the original. }
...
{ DysCopyWaveformSelection/DysGetWaveClipboard/WaveClipboard* vars,
  DysWarpModeName/DysWaveGridStepFrames, the whole TDysWaveformContent
  implementation, and DysSetWaveformClip were removed here - see this
  unit's interface-section comment on why (ported to DysMptiApp.pas,
  unreachable regardless). BuildAddEffectMenu, right below, stays - it's
  still used by DysTrackerApp's own effects pane. }
